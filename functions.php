...
function bloggerwp_setup() {
	...
	// Added WooCommerce Support, Product Image Zoom Support, Product Gallery Support, Product Gallery Lightbox Support
...
// Shop Per Page Limit
add_filter( 'loop_shop_per_page', 'bloggerwp_shop_per_page', 20 );
function bloggerwp_shop_per_page( $cols ) {
  $cols = 9;
  return $cols;
}

// Conditional Script for Right Click 
$GLOBALS['bloggerwp_theme_options']  = bloggerwp_get_theme_options();
$bloggerwp_right_click_disable= $bloggerwp_theme_options['bloggerwp-right-click-disable-option'];

if($bloggerwp_right_click_disable==1){ 
	function bloggerwp_options_scripts() {
		wp_enqueue_script( 'bloggerwp-option-script', get_template_directory_uri() . '/assets/js/option-script.js', array(), bloggerwp_version, true );
	}
